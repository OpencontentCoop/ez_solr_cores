...
# v.2.1 Nov 2015, per nuovi solr (nuovo metodo per tenere i core)
# Changelog
# cambiano posizione cores
#   /home/solr/multicore-8990/cores =>  /home/solr/multicore-8990/java/solr
#   /home/solr/multicore-8990/cores/solr.xml =>  /home/solr/multicore-8990/java/solr/solr.xml
#   /home/solr/multicore-8990/cores/cestino =>  /home/solr/multicore-8990/java/solr/_cestino
# E' necessario, una volta per tutte (quando si crea un nuovo multicore) 
#   * avere la directory /java/solr/ezp-default/conf, mentre /java/solr/ezp-default/data deve essere eliminato,
#   * si deve TOGLIERE il core ezp-default *A MANO* dal file /java/solr/solr.xml
#   * si elimina  defaultCoreName= da /java/solr/solr.xml
# es. 
#    rm -rf /home/solr/multicore-9001/java/solr/ezp-default/data
#    vim /home/solr/multicore-9001/java/solr/solr.xml  # elimino riga:  <core name="ezp-default" instanceDir="ezp-default" />, ed elimino defaultCoreName=
...
case $action in
  add)
    if ! [ -f "$SOLR_MULTICORES_DIR/java/solr/ezp-default/conf/schema.xml" ] ; then
      echo "ERRORE: $SOLR_MULTICORES_DIR/java/solr/ezp-default/conf/schema.xml mancante:  in $SOLR_MULTICORES_DIR/java/ e' necessario avere una copia di extension/ezfind/java/"
    ...
    if [ -d "$SOLR_MULTICORES_DIR/java/solr/$CORE_NAME" ] ; then
      echo "ERRORE: core '$CORE_NAME' gia' esistente"
      exit 1
    fi


    mkdir $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME

    mkdir -p $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME/data
    # creo /conf a partire da un template
    mkdir $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME/conf
    cp -ar $SOLR_MULTICORES_DIR/java/solr/ezp-default/conf/*  $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME/conf/
    # modifico dataDir
    sed -i "s/solr.data.dir:.\/solr\/data/solr.data.dir:..\/java\/solr\/$CORE_NAME\/data/" $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME/conf/solrconfig.xml
    
    # modifico solr.xml
    #cp -a $SOLR_MULTICORES_DIR/cores/solr.xml $SOLR_MULTICORES_DIR/cores/solr.xml.copia_ante_aggiunta_core_del_$DATE
    cp -a $SOLR_MULTICORES_DIR/java/solr/solr.xml $SOLR_MULTICORES_DIR/java/solr/solr.xml.copia_ante_aggiunta_core_del_$DATE
    sed -i "s/<\/cores>/     <core name=\"$CORE_NAME\" instanceDir=\"$CORE_NAME\" \/>\n    <\/cores>/" $SOLR_MULTICORES_DIR/java/solr/solr.xml
    echo
    echo "OK, il core '$CORE_NAME' e' stato CREATO in: $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME"
    echo 
    echo "*******************************************"
    echo
    ...
    echo "  $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME/data"
    echo 
  ...
  remove) 
    ...
    trash_dir=$SOLR_MULTICORES_DIR/java/solr/_cestino/$DATE/
    echo "Elimino directory con data e configurazione (messe in cestino: $trash_dir)"
    mkdir -p $trash_dir
    mv -f $SOLR_MULTICORES_DIR/java/solr/$CORE_NAME $trash_dir 
    # modifico solr.xml
    #solr_xml_trash_folder=$SOLR_MULTICORES_DIR/cores/cestino/old_solr.xml
    solr_xml_trash_folder=$SOLR_MULTICORES_DIR/java/solr/_cestino/old_solr.xml
    mkdir -p $solr_xml_trash_folder
    nomefile_backup=$solr_xml_trash_folder/solr.xml.copia_ante_aggiunta_core_del_$DATE
    echo "Elimino il core dal file  $SOLR_MULTICORES_DIR/java/solr/solr.xml (conservo copia in $nomefile_backup)"
    cp -a $SOLR_MULTICORES_DIR/java/solr/solr.xml $nomefile_backup
    sed -i "/instanceDir=\"$CORE_NAME\" \/>/d" $SOLR_MULTICORES_DIR/java/solr/solr.xml
    echo
  ...
  list)
    for solr_multicore_name in $SOLR_MULTICORES_NAMES ; do 
      ...
      [ $VERBOSE ] && echo "Lucene Version  (cat $solr_multicore_dir/java/solr/ezp-default/conf/solrconfig.xml | grep '<luceneMatchVersion>')"
      [ $VERBOSE ] && cat $solr_multicore_dir/java/solr/ezp-default/conf/solrconfig.xml | grep '<luceneMatchVersion>'
      echo "Cores list:"
      [ $VERBOSE ] && echo "cat $solr_multicore_dir/java/solr/solr.xml | grep instanceDir )"
      cat $solr_multicore_dir/java/solr/solr.xml | grep instanceDir
      if [ $? -eq 1 ] ; then 
        echo "         none"
      fi
    done
  ...
  status)
    for solr_multicore_name in $SOLR_MULTICORES_NAMES ; do
      ...
      #  es. http://localhost:9001/solr/mycore/admin/ping
      CORE_NAMES=`cat $solr_multicore_dir/java/solr/solr.xml | grep "<core name="  | sed  's/[[:blank:]]*<core name=\"//g' |  sed  's/".*//g'`
      SOLR_MULTICORES_PORT=${solr_multicore_dir:$(( ${#solr_multicore_dir}-4 ))}
      #SOLR_MULTICORES_INIT_SCRIPT=solr-$solr_multicore_name
      SOLR_MULTICORES_INIT_SCRIPT=$solr_multicore_name
